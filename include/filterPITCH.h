...
int		P1_pitch = 6; // watch out! if P1_roll is higher then C2_P_BIAS_UPDATE then things will go wrong
int		P2_pitch = 10; // watch out! if P2_roll is higher then C2_P_BIAS_UPDATE then things will go wrong
...
int pitchControl(int pitchRate,int pitch_angle, int P_js){
	...
	//     calculate stabilization
	    P_stabilize = P_js + DECREASE_SHIFT(0-P_angle,C2_P_BIAS_UPDATE - P1_pitch)
